...
<main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
    ...
    <div class="container-fluid py-4 px-5">
        ...
        {{-- ALERT SYSTEM (Jika Ada Error Misal Kelas Kosong) --}}
        @if (session('error'))
            <div class="alert bg-gradient-danger alert-dismissible text-white fade show" role="alert">
                <span class="text-sm"><strong>Peringatan!</strong> {{ session('error') }}</span>
        ...
        <div class="card shadow-sm border mb-4">
            <div class="card-body p-3">
                <form id="filterFormWali" action="{{ route('walikelas.monitoring.wali') }}" method="GET" class="row g-3 align-items-end mb-0">
                    <div class="col-md-4">
                        <label class="form-label fw-bold text-xs text-uppercase text-secondary">Pilih Kelas</label>
                        <select name="id_kelas" class="form-select border-secondary ps-2" onchange="showLoading('Memuat kelas...');">
                            {{-- Jika tidak ada kelas, munculkan pesan --}}
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="semester" class="form-select border-secondary ps-2" onchange="showLoading('Memuat data semester...');">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="tahun_ajaran" class="form-select border-secondary ps-2" onchange="showLoading('Memuat tahun ajaran...');">
                            @foreach($tahunAjaranList as $ta)
                                <option value="{{ $ta }}" {{ $tahun_ajaran == $ta ? 'selected' : '' }}>{{ $ta }}</option>
                            @endforeach
                        </select>
                    </div>
                    {{-- Hidden Submit untuk Handle Event Onchange --}}
                    <button type="submit" class="d-none"></button>
                </form>
            </div>
        </div>

        {{-- JIKA DATA KELAS DITEMUKAN, TAMPILKAN UI BERIKUT --}}
        @if($dataKelas)
...
<script>
    ...
    // FUNGSI LOADING UNTUK FILTER DROPDOWN
    function showLoading(textMsg = 'Memuat data...') {
        $('#loadingOverlay span').text(textMsg);
    ...
    function prosesGenerate() {
        if (confirm('Apakah Anda yakin data sudah lengkap? Data Rapor akan digenerate dan dikunci.')) {
            $('#loadingOverlay span').text('Sedang memproses penyusunan rapor...');
            $('#loadingOverlay').attr('style', 'display: flex !important; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.7); justify-content: center; align-items: center; color: white; font-size: 1.5rem; z-index: 999999;');
