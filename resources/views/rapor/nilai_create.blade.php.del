{{-- File: resources/views/rapor/nilai_create.blade.php --}}

@extends('layouts.app')

@section('title', 'Input Nilai Akhir Baru')
                    ...
                    <div class="card my-4 shadow-xs border">
                        ...
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                                <h6 class="text-white text-capitalize ps-3 mb-0"><i class="fas fa-plus me-2"></i> Input Nilai Akhir Baru (Nilai Rapor)</h6>
                            </div>
                        </div>
                        ...
                        <div class="card-body px-0 pb-2">
                            ...
                            <div class="p-4 border-bottom">
                                ...
                                <form action="{{ route('nilai.input.akhir') }}" method="GET" class="row align-items-end">
                                    ...
                                    <div class="col-md-2 mb-3">
                                        <button type="submit" class="btn btn-primary w-100 mb-0">Tampilkan Siswa</button>
                                    </div>
                                </form>
                            </div>
                            
                            {{-- FORM INPUT NILAI (Form ini menggunakan method POST untuk simpan) --}}
                            @if ($siswa->isNotEmpty() && $request->id_kelas && $request->id_mapel && $request->id_tahun_ajaran && $request->semester)
                                <form action="{{ route('nilai.simpan.akhir') }}" method="POST">
                                        ...
                                        <table class="table align-items-center mb-0">
                                            ...
                                            <tbody>
                                                @foreach ($siswa as $index => $s)
                                                    @php
                                                        // Ambil data nilai yang sudah ada (untuk mode EDIT jika ada)
                                                        $nilaiLama = $rapor->get($s->id_siswa);
                                                        // Jika ada nilai lama, muat. Jika tidak, ambil dari old() atau kosong.
                                    ...
                                    <div class="card-footer text-end pt-0 border-top mt-4">
                                        <button type="submit" class="btn bg-gradient-success mt-4 mb-0"><i class="fas fa-save me-2"></i> Simpan Nilai Akhir</button>
                                    </div>
                                </form>
                            ...
                            @else
                                <p class="text-secondary text-center text-sm my-3">
                                    Silakan pilih Kelas, Mata Pelajaran, Tahun Ajaran, dan Semester untuk memulai input nilai.
                                </p>
                            @endif
